...
public:
		...
		: SceneBase{ init }
	{
		...
		clearMenu.addButton([this] {this->changeScene(U"TestScene", .3s); }).setText(U"次のステージ").setRectPos(Scene::Center().x - 100, 360);
		...
		failedMenu.addButton([this] {this->changeScene(U"TestScene", .3s); }).setText(U"リトライ").setRectPos(Scene::Center().x - 100, 360);
